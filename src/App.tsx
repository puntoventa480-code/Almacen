...
// Declare google types for window and File System Access API
declare global {
  interface Window {
    google: any;
    gapi: any;
...
export default function App() {
  ...
  // Initialize manual IDs from config on load
  useEffect(() => {
    if (config.googleDriveFolderId) setManualFolderId(config.googleDriveFolderId);
    if (config.googleDriveBackupFileId) setManualFileId(config.googleDriveBackupFileId);
  }, [config.googleDriveFolderId, config.googleDriveBackupFileId]);
  ...
  // --- Google Drive Logic ---
  // (Logic kept identical to previous version, condensed for brevity in this specific file update focus)
  useEffect(() => {
    const loadGapi = () => {
      if (window.gapi) {
    ...
    if (!window.gapi) {
      const interval = setInterval(() => {
        if (window.gapi) { clearInterval(interval); loadGapi(); }
      }, 500);
    } else { loadGapi(); }
  }, []);
  ...
  const uploadToDrive = async (isManual = false) => {
    ...
    try {
       ...
       if (!fileId) {
         ...
         const response = await window.gapi.client.drive.files.list({ q: query, fields: 'files(id)' });
       ...
       if (res.ok) {
         ...
         setConfig(prev => ({ ...prev, googleDriveBackupFileId: json.id, lastSync: new Date().toISOString() }));
  ...
  const checkForRemoteUpdates = async () => {
    ...
    try {
      ...
      if (fileId) {
         try {
             const res = await window.gapi.client.drive.files.get({ fileId: fileId, fields: 'id, modifiedTime' });
             remoteFile = res.result;
         } catch (err) {}
      }
      if (!remoteFile) {
        const fileName = 'gestor_pro_backup.json';
        let query = `name = '${fileName}' and trashed = false`;
        if (config.googleDriveFolderId) query += ` and '${config.googleDriveFolderId}' in parents`;
        const response = await window.gapi.client.drive.files.list({ q: query, fields: 'files(id, modifiedTime)' });
        if (response.result.files && response.result.files.length > 0) {
            remoteFile = response.result.files[0];
            setConfig(prev => ({...prev, googleDriveBackupFileId: remoteFile.id}));
        }
      }
      if (remoteFile) {
        ...
        if (remoteTime > localTime + 60000) {
           if (window.confirm("Se han encontrado datos más recientes en Google Drive. ¿Deseas restaurarlos?")) {
      ...
      if (data) {
        ...
        if (data.config) {
             setConfig(prev => ({ 
                ...
                googleDriveFolderId: prev.googleDriveFolderId,
                googleDriveBackupFileId: prev.googleDriveBackupFileId 
            }));
        }
        alert("Datos restaurados correctamente.");
      }
    } catch (e) { alert("Error al descargar copia de seguridad."); }
  };

  const handleDriveConnect = () => {
    if (tokenClient) tokenClient.requestAccessToken({ prompt: 'consent' });
    else alert("Configura tu Client ID primero en Ajustes.");
  };
  
  const handleSaveDriveConfig = () => {
      setConfig(prev => ({
          ...prev,
          googleDriveClientId: manualKeyInput || prev.googleDriveClientId,
          googleDriveFolderId: manualFolderId || undefined,
          googleDriveBackupFileId: manualFileId || undefined
      }));
      setManualKeyInput('');
      alert("Configuración de Drive guardada.");
  };

  // --- Local Backup Logic ---
  
  // Helper to apply data state
  ...
  // --- Smart File System Logic (Advanced) ---
  const handleSmartSave = async () => {
    if (!('showSaveFilePicker' in window)) {
        alert("Tu navegador no soporta esta función. Usa 'Descargar Archivo'.");
        return;
    }
    try {
        const handle = await window.showSaveFilePicker({
            suggestedName: `gestor-pro-backup-${new Date().toISOString().split('T')[0]}.json`,
            types: [{
        ...
        await writable.write(JSON.stringify(getSyncData(), null, 2));
        await writable.close();
        alert("Respaldo guardado exitosamente en la ubicación seleccionada.");
    } catch (err) {
  ...
  const handleSmartLoad = async () => {
    if (!('showDirectoryPicker' in window)) {
        alert("Tu navegador no soporta esta función. Usa 'Cargar Archivo'.");
        return;
    }
    try {
        // 1. Pick Directory
        ...
        // 2. Scan for JSONs
        let newestFile: { handle: any, date: number, name: string } | null = null;
        
        // Iterate directory (async iterator)
        for await (const entry of (dirHandle as any).values()) {
            if (entry.kind === 'file' && entry.name.endsWith('.json')) {
                const file = await entry.getFile();
                // Check if it looks like our backup (optional check) or just use latest
                if (file.lastModified > (newestFile?.date || 0)) {
                    newestFile = { handle: entry, date: file.lastModified, name: entry.name };
                }
            }
        }

        if (newestFile) {
            if (window.confirm(`Archivo más reciente encontrado: ${newestFile.name} (${new Date(newestFile.date).toLocaleString()}). \n¿Deseas restaurar estos datos?`)) {
                const file = await newestFile.handle.getFile();
                const text = await file.text();
                const json = JSON.parse(text);
                restoreData(json);
                alert("Datos restaurados desde el archivo más reciente.");
            }
        } else {
        ...
        console.error(err);
        alert("No se pudo acceder a la carpeta o el usuario canceló.");
    }
  };
  ...
  const renderAI = () => (
    <div className="h-[calc(100vh-140px)] flex flex-col animate-in fade-in duration-300">
       <div className="bg-white p-3 border-b border-slate-100 flex items-center justify-between shadow-sm z-10 flex-shrink-0">
          ...
          <button onClick={() => { if(window.confirm("¿Desconectar API Key manual?")) setCustomApiKey(''); }} className={`flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-bold transition-all ${!customApiKey ? 'hidden' : ''}`}>
            ...
            <div className="flex flex-col items-center justify-center p-8 text-center bg-white m-4 rounded-2xl border border-slate-100 shadow-sm max-w-xl mx-auto mt-10">
                ...
                <div className="w-full space-y-6">
                    <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                      ...
                      <button onClick={() => ((window as any).aistudio as AIStudio).openSelectKey()} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-indigo-200">
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                        ...
                        <div className="flex gap-2">
                           <input type="password" value={manualKeyInput} onChange={(e) => setManualKeyInput(e.target.value)} placeholder="sk-..." className="flex-1 p-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"/>
                           <button onClick={handleSaveManualKey} disabled={!manualKeyInput} className="bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-white font-bold px-4 rounded-xl flex items-center gap-2"><Save size={18} /></button>
                        </div>
                    </div>
                </div>
            </div>
             ...
             <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
                {aiMessages.map((m, i) => (
                  <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                     <div className={`max-w-[80%] p-3 rounded-2xl ${m.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white border border-slate-200 text-slate-700 rounded-tl-none shadow-sm'}`}>{m.text}</div>
                  </div>
                ))}
                {isAiLoading && <div className="flex justify-start"><div className="bg-white border border-slate-200 p-3 rounded-2xl rounded-tl-none shadow-sm flex items-center gap-2"><Loader2 size={16} className="animate-spin text-indigo-500" /><span className="text-xs text-slate-400">Pensando...</span></div></div>}
             </div>
             <div className="p-4 bg-white border-t border-slate-100 flex gap-2">
                <input value={aiInput} onChange={(e) => setAiInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAiSendMessage()} placeholder="Escribe tu consulta..." className="flex-1 p-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50"/>
                <button onClick={handleAiSendMessage} disabled={isAiLoading || !aiInput.trim()} className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white p-3 rounded-xl transition-colors"><Send size={20} /></button>
             </div>
         </div>
       )}
    </div>
  ...
  const renderSettings = () => (
    <div className="animate-in fade-in duration-500 space-y-6">
      ...
      {/* Drive Sync (Collapsed for brevity, logic exists) */}
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
         <div className="flex items-center gap-3 mb-4">
             <div className="p-2 bg-blue-50 rounded-lg text-blue-600"><Cloud size={24} /></div>
             <div><h3 className="text-lg font-bold text-slate-800">Sincronización en la Nube</h3><p className="text-sm text-slate-500">Google Drive</p></div>
         </div>
         <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
            {!config.googleDriveClientId ? (
               <div className="space-y-4">
                  <div>
                    ...
                    <input value={manualKeyInput} onChange={(e) => setManualKeyInput(e.target.value)} placeholder="Ej. 123...apps.googleusercontent.com" className="w-full p-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"/>
                  </div>
                  <button onClick={() => { setConfig(prev => ({ ...prev, googleDriveClientId: manualKeyInput })); setManualKeyInput(''); }} className="bg-blue-600 text-white px-4 py-2 rounded-xl font-bold text-sm">Guardar ID</button>
               </div>
            ) : (
               <div className="space-y-4">
                  <div className="flex items-center justify-between">
                     <span className="font-bold text-slate-700">Estado: {userInfo?.connected ? 'Conectado' : 'Esperando...'}</span>
                     {config.lastSync && <span className="text-xs text-slate-400">Última Sync: {new Date(config.lastSync).toLocaleString()}</span>}
                  </div>
                  <div className="flex gap-2">
                     {!userInfo?.connected ? (
                        <button onClick={handleDriveConnect} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-bold text-sm"><CloudLightning size={16} /> Conectar</button>
                     ) : (
                       <>
                         <button onClick={() => uploadToDrive(true)} disabled={isSyncing} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-xl font-bold text-sm disabled:opacity-50">{isSyncing ? <Loader2 size={16} className="animate-spin"/> : <UploadCloud size={16} />} Subir</button>
                         <button onClick={checkForRemoteUpdates} disabled={isSyncing} className="flex items-center gap-2 bg-slate-200 hover:bg-slate-300 text-slate-700 px-4 py-2 rounded-xl font-bold text-sm disabled:opacity-50"><DownloadCloud size={16} /> Restaurar</button>
                       </>
                     )}
                     <button onClick={() => { if(window.confirm("¿Desvincular?")) { setConfig(prev => ({ ...prev, googleDriveClientId: undefined })); setUserInfo(null); } }} className="ml-auto text-rose-500 hover:bg-rose-50 px-3 py-2 rounded-xl"><LogOut size={18} /></button>
                  </div>
               </div>
            )}
         </div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
         <div className="flex items-center gap-3 mb-4">
             ...
             <div>
               ...
               <p className="text-sm text-slate-500">Gestiona tus copias de seguridad manualmente.</p>
             </div>
         </div>
         ...
         <div className="space-y-4">
             ...
             <div className="bg-amber-50 p-4 rounded-xl border border-amber-100">
                <p className="text-xs font-bold text-amber-800 uppercase mb-3 flex items-center gap-1"><Folder size={12}/> Opciones Avanzadas (Escritorio)</p>
                <div className="flex flex-wrap gap-3">
                   <button 
                     onClick={handleSmartSave}
                     className="flex items-center gap-2 bg-amber-600 hover:bg-amber-700 text-white px-4 py-3 rounded-xl font-bold text-sm transition-colors shadow-sm"
                   >
                     <Save size={18} /> Guardar Como...
                   </button>
                   <button 
                     onClick={handleSmartLoad}
                     className="flex items-center gap-2 bg-amber-100 hover:bg-amber-200 text-amber-800 px-4 py-3 rounded-xl font-bold text-sm transition-colors border border-amber-200"
                   >
                     <FolderOpen size={18} /> Escanear Carpeta (Cargar Reciente)
                   </button>
                </div>
                <p className="text-[10px] text-amber-700/60 mt-2">
                   Permite elegir una carpeta específica y detecta automáticamente el archivo de respaldo más nuevo.
                </p>
             </div>

             {/* Legacy Fallback */}
             <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                <p className="text-xs font-bold text-slate-400 uppercase mb-3 flex items-center gap-1"><Archive size={12}/> Opciones Básicas</p>
                <div className="flex flex-wrap gap-3">
                    <button 
                    onClick={handleExportData}
                    className="flex items-center gap-2 bg-white hover:bg-slate-100 text-slate-700 px-4 py-2 rounded-xl font-bold text-sm transition-colors border border-slate-200 shadow-sm"
                    >
      ...
      <div className="bg-slate-900 text-slate-300 p-6 rounded-2xl shadow-lg font-mono">
        <div className="flex items-center gap-3 mb-6 border-b border-slate-700 pb-4">
          ...
          <h2 className="text-xl font-bold text-white">Consola de Configuración</h2>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 mb-3">Estado Actual</h3>
            <pre className="bg-slate-950 p-4 rounded-xl overflow-x-auto text-xs leading-relaxed border border-slate-800">{JSON.stringify(config, null, 2)}</pre>
          </div>
          <div>
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 mb-3">Inyectar JSON</h3>
            <textarea className="w-full h-48 bg-slate-800 text-emerald-300 border border-slate-700 rounded-xl p-4 font-mono text-xs focus:ring-2 focus:ring-emerald-500 focus:outline-none mb-4" placeholder='{ "currencySymbol": "€", ... }' value={apiJsonInput} onChange={(e) => setApiJsonInput(e.target.value)}></textarea>
            <button onClick={handleApiUpdate} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2"><Save size={18} /> Aplicar Cambios</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="flex h-screen bg-slate-50 text-slate-900">
      ...
      <aside className="w-20 lg:w-64 bg-slate-900 flex flex-col items-center lg:items-stretch py-6 flex-shrink-0 transition-all z-20">
        <div className="mb-8 px-4 flex items-center justify-center lg:justify-start gap-3">
          <div className="w-10 h-10 bg-indigo-500 rounded-xl flex items-center justify-center text-white font-bold shadow-lg shadow-indigo-500/30">GP</div>
          <span className="text-xl font-bold text-white hidden lg:block tracking-tight">Gestor Pro</span>
        </div>
        <nav className="flex-1 space-y-2 px-2">
          <button onClick={() => setCurrentView('dashboard')} className={`w-full p-3 rounded-xl flex items-center gap-3 transition-all ${currentView === 'dashboard' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`} title="Dashboard"><LayoutDashboard size={22} /><span className="hidden lg:block font-medium">Dashboard</span></button>
          <button onClick={() => setCurrentView('pos')} className={`w-full p-3 rounded-xl flex items-center gap-3 transition-all ${currentView === 'pos' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`} title="Punto de Venta"><ShoppingCart size={22} /><span className="hidden lg:block font-medium">Vender</span></button>
          <button onClick={() => setCurrentView('inventory')} className={`w-full p-3 rounded-xl flex items-center gap-3 transition-all ${currentView === 'inventory' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`} title="Inventario"><Package size={22} /><span className="hidden lg:block font-medium">Inventario</span></button>
          <button onClick={() => setCurrentView('debts')} className={`w-full p-3 rounded-xl flex items-center gap-3 transition-all ${currentView === 'debts' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`} title="Deudas"><CreditCard size={22} /><span className="hidden lg:block font-medium">Deudas</span></button>
          <button onClick={() => setCurrentView('ai')} className={`w-full p-3 rounded-xl flex items-center gap-3 transition-all ${currentView === 'ai' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`} title="Asistente AI"><Bot size={22} /><span className="hidden lg:block font-medium">Asistente AI</span></button>
        </nav>
        <div className="px-2 mt-auto">
          <button onClick={() => setCurrentView('settings')} className={`w-full p-3 rounded-xl flex items-center gap-3 transition-all ${currentView === 'settings' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:bg-slate-800 hover:text-slate-300'}`} title="Configuración"><Settings size={22} /><span className="hidden lg:block font-medium">Ajustes</span></button>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto h-full relative">
        <header className="bg-white border-b border-slate-100 p-6 flex justify-between items-center sticky top-0 z-10 backdrop-blur-md bg-white/80">
          <div className="flex items-center gap-4">
             <div>
               <h1 className="text-2xl font-black text-slate-800 capitalize tracking-tight">{currentView === 'pos' ? 'Punto de Venta' : currentView === 'ai' ? 'Asistente Virtual' : currentView}</h1>
               <p className="text-sm text-slate-400 font-medium">{config.shopName} - {new Date().toLocaleDateString('es-MX', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
             </div>
             {userInfo?.connected && <div className="flex items-center gap-2 px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-xs font-bold animate-in fade-in">{isSyncing ? <Loader2 size={12} className="animate-spin" /> : <Cloud size={12} />}{isSyncing ? 'Sincronizando...' : 'Nube Activa'}</div>}
          </div>
          <div className="flex items-center gap-3">
             <div className="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-700 font-bold">AD</div>
          </div>
        </header>
      ...
      {/* Modals are kept the same, just rendering them */}
      <Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setPreviewImage(null); }} title={currentView === 'inventory' ? (editingItem ? 'Editar Producto' : 'Nuevo Producto') : (editingItem ? 'Editar Deuda' : 'Nueva Deuda')}>
          ...
          <form onSubmit={handleSaveProduct} className="space-y-4">
             <div className="flex flex-col items-center mb-4">
                <label className="relative w-32 h-32 bg-slate-100 rounded-2xl flex flex-col items-center justify-center cursor-pointer overflow-hidden border-2 border-dashed border-slate-300 hover:bg-slate-50 transition-colors group">
                  {previewImage ? <img src={previewImage} alt="Preview" className="w-full h-full object-cover" /> : <><Camera className="text-slate-400 mb-2 group-hover:text-slate-600" size={32} /><span className="text-xs text-slate-400 font-medium text-center px-2">Subir Foto</span></>}
                  <input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />
                </label>
             </div>
            <div className="grid grid-cols-2 gap-4">
               <div><label className="block text-sm font-bold text-slate-700 mb-1">Código SKU</label><div className="flex gap-2"><input name="sku" required value={formSku} onChange={(e) => setFormSku(e.target.value)} className="w-full p-3 rounded-xl border border-slate-200 bg-slate-50 font-mono text-sm" placeholder="AUTO-GEN" /><button type="button" onClick={() => setFormSku(generateSKU())} className="p-3 bg-slate-200 rounded-xl hover:bg-slate-300" title="Generar Nuevo"><RefreshCw size={18}/></button></div></div>
               <div><label className="block text-sm font-bold text-slate-700 mb-1">Categoría</label><input name="category" required defaultValue={editingItem?.category} list="categories-list" className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Seleccionar o escribir..." /><datalist id="categories-list">{config.categories.map(c => <option key={c} value={c} />)}</datalist></div>
            </div>
            <div><label className="block text-sm font-bold text-slate-700 mb-1">Nombre del Producto</label><input name="name" required defaultValue={editingItem?.name} className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Ej. Camiseta Algodón" /></div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm font-bold text-slate-700 mb-1">Stock</label><input name="quantity" type="number" required defaultValue={editingItem?.quantity} className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0" /></div>
              <div><label className="block text-sm font-bold text-slate-700 mb-1">Precio Venta</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold">{config.currencySymbol}</span><input name="price" type="number" step="0.01" required defaultValue={editingItem?.price} className="w-full pl-8 p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0.00" /></div></div>
            </div>
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-200"><label className="block text-sm font-bold text-slate-700 mb-1 flex items-center gap-2"><Box size={16} /> Unidades por Caja (Defecto)</label><input name="defaultBulkSize" type="number" min="1" defaultValue={editingItem?.defaultBulkSize || 12} className="w-full p-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Ej. 12" /><p className="text-[10px] text-slate-500 mt-1">Se usará como valor predeterminado al vender o agregar stock por caja.</p></div>
            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl mt-4 flex items-center justify-center gap-2"><Save size={20} /> Guardar Producto</button>
          </form>
        ) : (
          <form onSubmit={handleSaveDebt} className="space-y-4">
             <div><label className="block text-sm font-bold text-slate-700 mb-1">Cliente</label><input name="debtorName" required defaultValue={editingItem?.debtorName} list="clients-list" className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-rose-500 outline-none" placeholder="Nombre del cliente..." autoComplete="off"/><datalist id="clients-list">{clients.map(c => <option key={c} value={c} />)}</datalist></div>
            <div><label className="block text-sm font-bold text-slate-700 mb-1">Descripción / Items</label><input name="description" required defaultValue={editingItem?.description} className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-rose-500 outline-none" placeholder="Ej. Compra a crédito..." /></div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm font-bold text-slate-700 mb-1">Monto</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold">{config.currencySymbol}</span><input name="amount" type="number" step="0.01" required defaultValue={editingItem?.amount} className="w-full pl-8 p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-rose-500 outline-none" placeholder="0.00" /></div></div>
              <div><label className="block text-sm font-bold text-slate-700 mb-1">Fecha Límite</label><input name="dueDate" type="date" required defaultValue={editingItem?.dueDate} className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-rose-500 outline-none" /></div>
            </div>
            <button type="submit" className="w-full bg-rose-600 hover:bg-rose-700 text-white font-bold py-3 rounded-xl mt-4 flex items-center justify-center gap-2"><Save size={20} /> Guardar Deuda</button>
          </form>
        )}
      </Modal>

      <Modal isOpen={isPaymentModalOpen} onClose={() => { setIsPaymentModalOpen(false); setPaymentAmount(''); }} title={`Abonar a ${selectedClient}`}>
        <form onSubmit={handlePartialPayment} className="space-y-4">
          <p className="text-sm text-slate-500">El abono se aplicará automáticamente a las deudas más antiguas primero.</p>
          <div><label className="block text-sm font-bold text-slate-700 mb-1">Monto a Abonar</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold">{config.currencySymbol}</span><input type="number" step="0.01" required value={paymentAmount} onChange={(e) => setPaymentAmount(e.target.value)} className="w-full pl-8 p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-xl font-bold text-slate-800" placeholder="0.00" autoFocus/></div></div>
          <button type="submit" className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3 rounded-xl mt-4 flex items-center justify-center gap-2"><Banknote size={20} /> Registrar Abono</button>
        </form>
      </Modal>

      <Modal isOpen={isStockEntryModalOpen} onClose={() => { setIsStockEntryModalOpen(false); setStockEntryQty(''); }} title={`Entrada de Stock: ${editingItem?.name}`}>
        <form onSubmit={handleStockEntry} className="space-y-4">
          <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 flex items-center gap-3"><div className="p-3 bg-emerald-100 rounded-full text-emerald-600"><Package size={24}/></div><div><p className="text-sm text-emerald-800 font-medium">Stock Actual</p><p className="text-2xl font-bold text-emerald-700">{editingItem?.quantity}</p></div></div>
          <div className="flex gap-2 justify-center mb-2">
            <button type="button" onClick={() => setStockEntryIsBulk(false)} className={`px-4 py-2 rounded-lg font-bold text-sm transition-colors ${!stockEntryIsBulk ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-500'}`}>Por Unidad</button>
            <button type="button" onClick={() => setStockEntryIsBulk(true)} className={`px-4 py-2 rounded-lg font-bold text-sm transition-colors ${stockEntryIsBulk ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-500'}`}>Por Caja / Paca</button>
          </div>
          <div className="flex gap-4">
             <div className="flex-1"><label className="block text-sm font-bold text-slate-700 mb-1">{stockEntryIsBulk ? 'Cantidad de Cajas' : 'Cantidad a Agregar'}</label><input type="number" min="1" required value={stockEntryQty} onChange={(e) => setStockEntryQty(e.target.value)} className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none text-xl font-bold text-center" placeholder="0" autoFocus/></div>
             {stockEntryIsBulk && (<div className="w-1/3"><label className="block text-sm font-bold text-slate-700 mb-1">Unidades/Caja</label><input type="number" min="1" required value={stockEntryBulkSize} onChange={(e) => setStockEntryBulkSize(Number(e.target.value))} className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none text-xl font-bold text-center" /></div>)}
          </div>
          {stockEntryIsBulk && stockEntryQty && <div className="text-center bg-slate-50 p-2 rounded-lg text-sm text-slate-600">Total a agregar: <strong>{parseInt(stockEntryQty) * stockEntryBulkSize}</strong> unidades</div>}
          <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl mt-4 flex items-center justify-center gap-2"><Plus size={20} /> Confirmar Entrada</button>
        </form>
      </Modal>

      <Modal isOpen={isHistoryModalOpen} onClose={() => setIsHistoryModalOpen(false)} title={`Historial: ${editingItem?.name}`}>
        <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
           {itemHistory.length === 0 ? <div className="text-center py-8 text-slate-400 italic">No hay movimientos registrados.</div> : itemHistory.map(h => (
                   <div key={h.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
                       <div><div className="flex items-center gap-2"><span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase ${h.type === 'entrada' ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'}`}>{h.type}</span><span className="text-xs text-slate-400">{new Date(h.date).toLocaleString()}</span></div><p className="font-bold text-slate-700 text-sm mt-1">{h.note}</p></div>
                       <div className="flex items-center gap-3"><span className={`text-lg font-bold ${h.type === 'entrada' ? 'text-emerald-600' : 'text-slate-600'}`}>{h.type === 'entrada' ? '+' : '-'}{h.quantity}</span><button onClick={() => handleDeleteHistoryItem(h)} className="text-slate-300 hover:text-rose-500 p-1" title="Eliminar registro (Revertir)"><Trash2 size={16}/></button></div>
                   </div>
           ))}
        </div>
      </Modal>

      <Modal isOpen={isTicketModalOpen} onClose={() => setIsTicketModalOpen(false)} title="Datos para Factura / Ticket">
        <div className="flex flex-col md:flex-row gap-6">
            <div className="flex-1 space-y-4">
                <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 mb-4"><p className="text-xs text-indigo-800 mb-2 font-medium">Estos datos son exclusivamente para la impresión del ticket y no afectan al cliente seleccionado en el sistema.</p></div>
                <div><label className="block text-sm font-bold text-slate-700 mb-1">Título del Ticket</label><input value={ticketConfig.title} onChange={(e) => setTicketConfig({...ticketConfig, title: e.target.value})} className="w-full p-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-indigo-900" placeholder="Ej. NOTA DE ENTREGA"/></div>
                <div><label className="block text-sm font-bold text-slate-700 mb-1">Nombre Cliente</label><input value={ticketConfig.name} onChange={(e) => setTicketConfig({...ticketConfig, name: e.target.value})} className="w-full p-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Dejar vacío para usar cliente del sistema"/></div>
                <div><label className="block text-sm font-bold text-slate-700 mb-1">CI / DNI</label><input value={ticketConfig.ci} onChange={(e) => setTicketConfig({...ticketConfig, ci: e.target.value})} className="w-full p-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Documento de Identidad"/></div>
                <div><label className="block text-sm font-bold text-slate-700 mb-1">Dirección</label><input value={ticketConfig.address} onChange={(e) => setTicketConfig({...ticketConfig, address: e.target.value})} className="w-full p-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Dirección fiscal"/></div>
                <button onClick={() => setIsTicketModalOpen(false)} className="w-full bg-indigo-600 text-white py-2 rounded-xl font-bold mt-4">Guardar y Volver</button>
            </div>
            <div className="w-full md:w-64 bg-slate-200 p-4 rounded-xl border border-slate-300 flex flex-col items-center">
                ...
                <div className="bg-white p-2 w-[58mm] min-h-[250px] shadow-sm text-[11px] font-[Courier] leading-tight text-black overflow-hidden relative">
                    <div className="text-center mb-2"><div className="text-base uppercase" style={{fontSize: '14px'}}>{config.shopName}</div><div className="uppercase my-1" style={{fontSize: '12px'}}>{ticketConfig.title}</div><div>Nº: {Math.floor(Math.random() * 1000)}</div><div>{new Date().toLocaleDateString('es-MX')}</div></div>
                    <div className="border-t border-dashed border-black my-1"></div>
                    <div className="mb-2"><div>CLI: {(ticketConfig.name || posClient || 'CONSUMIDOR FINAL').toUpperCase()}</div>{ticketConfig.ci && <div>CI: {ticketConfig.ci}</div>}</div>
                    <div className="border-t border-dashed border-black my-1"></div>
                    <div className="space-y-2">{Object.keys(cart).length === 0 ? <div className="text-slate-400 italic text-center py-2">-- Vacío --</div> : Object.entries(cart).map(([id, item]) => { const p = products.find(prod => prod.id === id); const cartItem = item as CartItem; return (<div key={id}><div className="uppercase">{p?.name.substring(0, 25)}</div><div className="flex justify-between"><span>{cartItem.quantity} x {formatNumber(cartItem.customPrice)}</span><span>{formatNumber(cartItem.quantity * cartItem.customPrice)}</span></div></div>); })}</div>
                    <div className="border-y border-dashed border-black my-2 py-1 flex justify-between text-sm" style={{fontSize: '14px'}}><span>TOTAL:</span><span>{formatCurrency(cartTotal, config.currencySymbol)}</span></div>
                    <div className="mt-8 text-center"><div className="border-t border-black w-3/4 mx-auto mb-1"></div><div className="text-[10px]">Firma / Recibí Conforme</div></div>
                </div>
            </div>
        </div>
      </Modal>
